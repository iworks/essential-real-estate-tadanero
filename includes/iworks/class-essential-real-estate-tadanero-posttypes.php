<?php
/**
 * Tadanero: Essential Real Estate Post Types Class
 *
 * This class handles the loading and management of custom post types
 * for the Tadanero: Essential Real Estate.
 *
 * @package    iWorks
...
class iworks_wordpress_plugin_posttypes {
	...
	public function __construct() {
		...
		foreach ( glob( $posttypes_classes_dir . 'class*.php' ) as $filename_with_path ) {
			 ...
			 */
			if ( ! preg_match( '/^class-essential-real-estate-tadanero-posttype-([a-z]+).php$/', $filename, $matches ) ) {
				continue;
			}
			 ...
			 */
			$filter = sprintf(
				'essential-real-estate-tadanero/load/posttype/%s',
				$posttype_name
			);
			...
			if ( apply_filters( $filter, false ) ) {
				 ...
				 */
				$class_name = sprintf( 'iworks_essential_real_estate_tadanero_posttype_%s', $posttype_name );

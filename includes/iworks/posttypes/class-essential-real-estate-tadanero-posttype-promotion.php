...
class iworks_essential_real_estate_tadanero_posttype_promo extends iworks_essential_real_estate_tadanero_posttype_base {
	...
	public function __construct() {
		...
		$this->meta_boxes[ $this->posttypes_names[ $this->posttype_name ] ] = array(
			'featured-url' => array(
				'title'  => __( 'URL Configuration', 'essential-real-estate-tadanero' ),
				'fields' => array(
					array(
						'name'  => 'target_button_url',
						'type'  => 'url',
						'label' => esc_html__( 'Target Button URL', 'essential-real-estate-tadanero' ),
					),
					array(
						'name'  => 'target_button_text',
						'label' => esc_html__( 'Target Button Text', 'essential-real-estate-tadanero' ),
					),
				),
			),
		);
	...
	public function column_add( $columns ) {
		$inserted = array(
			'menu_order' => __( 'Order', 'essential-real-estate-tadanero' ),
		);
		$columns  = array_merge(
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) {
				...
				if ( empty( $button_label_more ) ) {
					$button_label_more = _x( 'Find out more', 'Promo button text', 'essential-real-estate-tadanero' );
				}
				$content .= sprintf(
					...
					esc_attr(
						sprintf(
							__( 'Article on: %s', 'essential-real-estate-tadanero' ),
							get_the_title()
						)
					),
	...
	public function action_init_register_post_type() {
		$labels = array(
			'name'                  => _x( 'Featured', 'Post Type General Name', 'essential-real-estate-tadanero' ),
			'singular_name'         => _x( 'Featured', 'Post Type Singular Name', 'essential-real-estate-tadanero' ),
			'menu_name'             => __( 'Featured', 'essential-real-estate-tadanero' ),
			'name_admin_bar'        => __( 'Featured', 'essential-real-estate-tadanero' ),
			'archives'              => __( 'Featured', 'essential-real-estate-tadanero' ),
			'all_items'             => __( 'Featured', 'essential-real-estate-tadanero' ),
			'add_new_item'          => __( 'Add New Featured', 'essential-real-estate-tadanero' ),
			'add_new'               => __( 'Add New', 'essential-real-estate-tadanero' ),
			'new_item'              => __( 'New Featured', 'essential-real-estate-tadanero' ),
			'edit_item'             => __( 'Edit Featured', 'essential-real-estate-tadanero' ),
			'update_item'           => __( 'Update Featured', 'essential-real-estate-tadanero' ),
			'view_item'             => __( 'View Featured', 'essential-real-estate-tadanero' ),
			'view_items'            => __( 'View Featured', 'essential-real-estate-tadanero' ),
			'search_items'          => __( 'Search Featured', 'essential-real-estate-tadanero' ),
			...
			'items_list'            => __( 'Featured list', 'essential-real-estate-tadanero' ),
			'items_list_navigation' => __( 'Featured list navigation', 'essential-real-estate-tadanero' ),
			'filter_items_list'     => __( 'Filter items list', 'essential-real-estate-tadanero' ),
		);
		$args   = array(
			'can_export'          => true,
			'capability_type'     => 'page',
			'description'         => __( 'Featured', 'essential-real-estate-tadanero' ),
			'exclude_from_search' => true,
			'has_archive'         => false,
			'hierarchical'        => false,
			'label'               => __( 'Featured', 'essential-real-estate-tadanero' ),
			'labels'              => $labels,
	...
	public function html_url( $post ) {
		...
		echo '<label><h4>';
		_e( 'Target URL', 'essential-real-estate-tadanero' );
		echo '</h4>';
		printf(
		...
		echo '<label><h4>';
		_e( 'Target Button Label', 'essential-real-estate-tadanero' );
		echo '</h4>';
		printf(
		...
		printf(
			...
			__( 'Leave empty to default', 'essential-real-estate-tadanero' )
		);
		/**
		 * is ukrainian text?
		 */
		...
		echo '<label><h4>';
		_e( 'Is it Ukrainian language?', 'essential-real-estate-tadanero' );
		echo '</h4><label>';
		printf(
			'<input type="checkbox" name="%s" value="uk" %s /> %s',
			esc_attr( $this->option_name_ukrainian ),
			checked( 'uk', $ukrainian, false ),
			esc_html__( 'Yes, it is in Ukrainian!', 'essential-real-estate-tadanero' )
		);
		echo '</label>';
	}

...
class iworks_essential_real_estate_tadanero_posttype_post extends iworks_essential_real_estate_tadanero_posttype_base {
	...
	public function __construct() {
		...
		$this->meta_boxes[] = array(
			'opinion-data' => array(
				'title'  => __( 'Post Gallery', 'essential-real-estate-tadanero' ),
				'fields' => array(
					array(
						'name'  => 'icon',
						'type'  => 'image',
	...
	public function get_related( $content, $post_ID ) {
		...
		$content .= sprintf( '<p class="related-posts-title">%s</p>', esc_html__( 'Related posts:', 'essential-real-estate-tadanero' ) );
	...
	public function html_post_gallery( $post ) {
		...
		printf(
			...
			esc_html__( 'Add picture', 'essential-real-estate-tadanero' )
		);
...
<div class="image-wrapper<?php echo esc_attr( empty( $src ) ? '' : ( 0 < $value ? ' has-file' : ' has-old-file' ) ); ?>">
	...
	<button type="button" class="image-reset" aria-label="<?php esc_attr_e( 'Remove file', 'essential-real-estate-tadanero' ); ?>"><span class="dashicons dashicons-trash"></span></button>

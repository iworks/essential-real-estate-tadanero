...
require_once dirname( ( dirname( __FILE__ ) ) ) . '/class-essential-real-estate-tadanero-base.php';

abstract class iworks_essential_real_estate_tadanero_posttype_base extends iworks_essential_real_estate_tadanero_base {
	...
	public function __construct() {
		...
		$this->posttypes_names  = apply_filters(
			'iworks/essential-real-estate-tadanero/posttypes_names/array',
			$this->posttypes_names
		);
		$this->taxonomies_names = apply_filters(
			'iworks/essential-real-estate-tadanero/taxonomies_names/array',
	...
	protected function get_select_array( $post_type, $atts = array() ) {
		...
		$list[0]   = __( '&mdash; Select &mdash;', 'essential-real-estate-tadanero' );
	...
	private function render_meta_image( $post, $one ) {
		...
		printf(
			...
			esc_attr__( 'Select Image', 'essential-real-estate-tadanero' ),
		);
		printf(
			'<input type="button" value="%s" class="button button-delete%s">',
			esc_attr__( 'Delete image', 'essential-real-estate-tadanero' ),
			empty( $value ) ? ' hidden' : ''
		);
		echo '</p>';
	}
	...
	private function render_meta_select( $post, $one ) {
		...
		printf( '<option value="">%s</option>', __( '&mdash; Select &mdash;', 'essential-real-estate-tadanero' ) );
	...
	private function setup_post_one_field( $post_id, $field, $meta_box_id ) {
		...
		return apply_filters(
			'iworks/essential-real-estate-tadanero/post/meta/field',
			$field,
			$post_id
		);
	}
	...
	public function filter_add_menu_order_column( $columns ) {
		$columns['menu_order'] = __( 'Order', 'essential-real-estate-tadanero' );
		return $columns;
	}
	...
	protected function get_taxonomy( $taxonomy_name ) {
		...
		return new WP_Error( 'taxonomy', esc_html__( 'Selected Taxonomy dosn\'t exists.', 'essential-real-estate-tadanero' ) );
	}
	...
	public function action_save_post_meta( $post_id, $post, $update ) {
		...
		foreach ( $this->meta_boxes[ $post_type ] as $group => $meta_box_data ) {
			...
			foreach ( $meta_box_data['fields'] as $field ) {
				...
				do_action( 'iworks/essential-real-estate-tadanero/postmeta/update', $post_id, $field, $key, $value );
			}
		}
	}
	...
	public function action_admin_enqueue_scripts_enqueue_assets() {
		$translation_array = array(
			...
			'l10n'            => array(
				'wp_media' => array(
					'title'  => esc_html__( 'Select or Upload Media', 'essential-real-estate-tadanero' ),
					'button' => array(
						'text' => esc_html__( 'Use this Media', 'essential-real-estate-tadanero' ),
					),
				),
			),
		);
		wp_localize_script(
			...
			'iworks_essential_real_estate_tadanero',
			apply_filters(
				'iworks/essential-real-estate-tadanero/wp_localize_script/admin',
				$translation_array
			)
		);
